...
const DefaultRules = ({ setDefaultRules }: { setDefaultRules: (strs: string[]) => void }) => {
    ...
    const getCheckBoxes = (config: { label: string; value: string }[], title: string) => {
        return (
            <div className={styles.defaultRulesWrap}>
                <div className={styles.title}>{title}</div>
                <div className={styles.rules}>
                    {config.map(({ label, value }) => (
                        <Checkbox key={label + value} value={value}>
                            {label}
                        </Checkbox>
                    ))}
                </div>
    ...
    return (
        <Checkbox.Group defaultValue={JSON.parse(localStorage.getItem('miku-block-default') || '[]')} onChange={changeCustom}>
